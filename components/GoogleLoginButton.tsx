...
export function GoogleLoginButton({ onAuthenticated }: GoogleLoginButtonProps) {
  ...
  const login = useGoogleLogin({
    ...
    onError: (errorResponse) => {
      const message = errorResponse.error_description ?? errorResponse.error ?? 'Error desconocido';
      setError(message);
      console.error('Google login failed:', errorResponse);
    },
    onNonOAuthError: (nonOAuthError) => {
      if (nonOAuthError.type === 'popup_failed_to_open') {
        setError('El navegador bloqueó la ventana emergente. Permite popups para este sitio e intenta de nuevo.');
      } else if (nonOAuthError.type === 'popup_closed') {
        setError('Cerraste la ventana de inicio de sesión. Intenta de nuevo.');
      } else {
  ...
  return (
    <div>
      <button
        ...
        onClick={() => {
          setError(null);
          if (scriptLoadedSuccessfully) login();
        }}
        type="button"
      ...
      >
        ...
        {scriptLoadedSuccessfully ? 'Conectar con Google Drive' : 'Cargando...'}
      </button>
      {error && (
